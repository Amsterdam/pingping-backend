...
class TaskUtil {
  ...
  static getUserTask(user: UserDocument, taskId: string): UserTask {
    const tasks: Array<UserTask> = user.tasks.filter(
      (t: UserTask) => t.taskId === taskId
    );
  ...
  static getCurrentUserTask(user: UserDocument): UserTask {
    const tasks: Array<UserTask> = user.tasks.filter(
      (t: UserTask) => t.status === TaskStatus.PendingUser
    );
  ...
  static updateUserTask(user: UserDocument, userTask: UserTask): UserDocument {
    let index = user.tasks
      .map((i: UserTask) => i.taskId)
      .indexOf(userTask.taskId);
  ...
  static addNextTaskToUser(user: UserDocument, taskId: string): UserDocument {
    ...
    const userTask: UserTask = new UserTask(
      taskDef.id,
  ...
  static getNextTaskId(answer: string, nextTask: string | object): string {
    ...
    return _.get(
      nextTask,
      answer.toLowerCase(),
      _.first(Object.values(nextTask))
    );
  }
  ...
  static async handleTask(
    ...
    const newStatus =
      taskDef.type === TaskType.YesOrNo && answer === "no"
    ...
    if (taskDef.nextTaskId) {
      user = this.addNextTaskToUser(
        user,
        this.getNextTaskId(answer, taskDef.nextTaskId)
      );
    }
